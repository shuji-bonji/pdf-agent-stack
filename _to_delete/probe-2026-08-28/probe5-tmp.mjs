...
console.log('chainStop', JSON.stringify(doc.chainStop), 'xref', doc.xref.size);
for (const [n, e] of doc.xref) {
  ...
  for (const key of ['FontFile','FontFile2','FontFile3']) {
    ...
    try {
      ...
      console.log('   resolve ->', s.kind, s.kind === 'stream' ? `keys: ${[...s.dict.entries.keys()].join(',')} raw=${s.raw.length}B` : '');
      if (s.kind === 'stream') { const d = await decodeStream(s); console.log('   decode ->', d.length, 'B'); }
    } catch (err) { console.log('   🔴 失敗:', String(err.message).slice(0,150)); }
  }
}
