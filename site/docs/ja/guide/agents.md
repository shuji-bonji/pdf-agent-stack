...
4 サーバーを登録するだけで、各サーバーが自己申告する **MCP instructions**（責務境界の宣言）が効きます。reader は「観測であり判定ではない」、verify は「反証はできるが証明はしない」、writer は「宣言は書けるが準拠は作れない」— これらの境界をエージェントが自然に守るよう設計されています。
