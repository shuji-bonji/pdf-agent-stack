...
タグ付き PDF のフォームを **PDF/UA-1 準拠へ修復**: Widget を Form 構造要素へ内包（7.18.4-1）、対象ページに /Tabs S（7.18.3-1）、フィールドに代替名 /TU（7.18.1-3）。**何度実行しても結果は同じ**（構造木に結ばれた Widget はスキップされる）。タグ無し文書は対象外（create 系の `tagged: true` で作り直すか `ensure_tagged` を先に）。
