...
各ツールの「プロンプト → 引数 → 返る JSON」は、[ツールリファレンス](/ja/reference/mcp/pdf-spec)の該当ツール末尾のアコーディオンにあります。JSON の形の読み方は [pdf-spec の出力の読み方](/ja/reference/pdf-spec-output) へ。シナリオ全体は[仕様調査](/ja/use-cases/spec-research)です。
