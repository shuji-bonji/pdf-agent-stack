...
veraPDF は PDF/A・PDF/UA を判定するが、**ISO 32000 本体の条文違反は見ない**。
そこが空いているため、次のような `shall` 違反がビューアの警告として「無害」に分類されたまま残ることがある。
...
テーブルは素の JSON なので、判定器を通さずに読むこともできる。
...
制約ごとに次のどれかを返す。**verdict（推奨判定）は出さない** —— それは pdf-verify の
[`evaluate_policy`](/ja/reference/mcp/pdf-verify#evaluate-policy) の役割である。
...
条文には、ファイル単体からは判定できない前提を持つものがある。たとえば R-9.9.2-2
...
**誰かが破った痕跡**であって、直近の書き手の違反とは限らない（§14.3.4 は既存の不整合を
そのまま残すことを許している）。そうした制約は `subjectNote` を持ち、レポートでも「違反の痕跡」と述べる。

## 単体で使う

MCP を立てずに CLI としても動く。CI に組み込む場合はこちらが軽い。
...
終了コードは 3 値である。**判定できなかったことを合格や違反に混ぜない**。
...
後から確かめられるようにするためである。テーブルと評価意味論は同一バージョンで結束している。

## やらないこと

- **条文原文の提供** — [pdf-spec](/ja/mcp/pdf-spec) の役割。テーブルは clause ID で参照するだけで原文を複製しない
- **適合の証明** — 反証だけができる
- **verdict・推奨判定** — [`evaluate_policy`](/ja/reference/mcp/pdf-verify#evaluate-policy) の役割
- **veraPDF の代替** — PDF/A の判定主体は veraPDF。収録は ISO 32000-1/-2 本体条文のみ
- **内容の真偽判定** — 条文に適合したファイルが嘘を述べることはある
