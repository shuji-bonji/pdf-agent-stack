...
::: info
**v0.11.1** の `tools/list` ハンドシェイクから自動生成（18 ツール・2026-08-12）。手で編集しない — 再生成は `node scripts/generate-reference.mjs`。日本語訳は翻訳メモリ（scripts/i18n）から適用され、原文が更新された項目は同期されるまで英語で表示される。
:::
