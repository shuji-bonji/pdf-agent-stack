...
::: info
**v0.6.0** の `tools/list` ハンドシェイクから自動生成（8 ツール・2026-09-01）。手で編集しない — 再生成は `node scripts/generate-reference.mjs`。日本語訳は翻訳メモリ（scripts/i18n）から適用され、原文が更新された項目は同期されるまで英語で表示される。
:::
