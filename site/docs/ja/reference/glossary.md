...
PDF Agent Stack の文書とツール出力に現れる用語。定義は ISO 規格の用語と衝突しないよう、規格側の意味に従う。

## 主張の強さに関わる語

この 3 つを混ぜないことが PDF Agent Stack 全体の設計思想である → [全体構成と責務](/ja/guide/architecture)。
...
PDF 規格の用語ではないが、本サイトが説明なしに使う語をここにまとめる。
...
| 反証 | 「正しい」と証明するのではなく「誤りを 1 つ示す」こと。適合は証明できず、反証だけができる |
...
本サイトの構成図（Mermaid）は、要素の種別を形で区別している。
