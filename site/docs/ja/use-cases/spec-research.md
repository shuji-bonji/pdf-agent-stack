...
実装や監査の判断を、記憶や検索エンジンではなく **ISO 32000 の原文**に着地させる。
pdf-spec が開くのは手元に置いた仕様コーパスだけで、**検証対象の PDF は開かない**。仕様が**何を要求するか**に答え、ファイルが**それを満たすか**は pdf-verify の仕事である。

以下は **2026-08-11 の実測クエリ**（受入監査で見つけた「署名後の増分更新」を条文に降ろした例）である。
...
（原本無傷・末尾追記 = タイムスタンプ付与）であることを原文で確認できる。

## 結果の読み方

- **ヒットなし = 「このコーパスは答えられない」であって「要求が存在しない」ではない。**
  ISO 19005（PDF/A）と ETSI PAdES はコーパス外（`list_specs` の coverage.gaps を読む）
- `get_requirements` の shall / should / may は要求の強さそのもの — 実装判断は shall を落とさない
  ことから始まる
- 条文を読んで分かるのは**仕様が何を要求するか**まで。目の前のファイルが満たすかは
  `validate_clauses` / `validate_conformance`（pdf-verify）で測る
- 宣言・適合・検証は別物 — 条文の「shall」を引用しても、ファイルの適合を証明したことにはならない
