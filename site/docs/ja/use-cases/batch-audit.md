...
受領フォルダに溜まった PDF をまとめて監査し、**問題のあるものだけ**に人の目を割り当てる。
まず全件に `evaluate_policy` を回してトリアージし、reject / human_review_required だけを
深掘りする — 全件深掘りは時間と文脈の無駄遣いになる。

以下は **2026-08-11 の実測**（5 検体一括・4 値判定がすべて出た回）である。
...
- **同じファイル・同じプロファイルなら常に同じ判定**。モデルにも実行日にも依らない —
  監査の再現性はルールエンジン側が担保する
- サマリ表の判定だけで仕分けし、**個票は問題ファイルにのみ付ける**（レポートの読み手も楽になる）
- プロファイルはファイル種別ごとに変えてよい（請求書 = financial・契約書 = contract）
- 全件 `use_with_caution` に寄るときは、trust_anchors 未指定が原因のことが多い —
  CA 証明書を一度入手すれば全件が identity 評価付きに変わる
