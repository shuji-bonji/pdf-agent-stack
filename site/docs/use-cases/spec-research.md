...
With pdf-spec-mcp v0.6.0, `list_specs` returned 17 documents, with ISO 19005-1..4 and ETSI EN 319 142-1/2 listed under `coverage.gaps`. The incremental update seen in the incoming audit was traced to the clauses with `get_requirements` (§7.5.6: 8 shall, 2 may, R-7.5.6-1), and `search_spec` for "document timestamp" returned 10 hits headed by §12.8.4.2 / §12.8.4.3 / §12.8.1. A search for `ISO 19005` returned 0 results, recorded as "this corpus cannot answer", not "there is no requirement". No path to the PDF under audit was passed to pdf-spec. On Grok Build, `${PDF_SPEC_DIR}` in the plugin's `env` is not expanded; an absolute path in `~/.grok/config.toml` made it work (→ [Environment variables](/reference/env-vars)). Report: [UC05.md](https://github.com/shuji-bonji/pdf-agent-stack/blob/main/eval/grok-eval/reports/UC05.md) (Japanese)
