...
With pdf-verify-mcp v0.26.0, `evaluate_policy` ran over 5 specimens (unsigned contract / unsigned invoice / unsigned report / gazette / tampered specimen) and produced all four verdicts: 1 `human_review_required`, 3 `use_with_caution` (`POL-CAUTION-UNSIGNED`, or TRUST-NOT-EVALUATED + REVOCATION-UNKNOWN) and 1 `reject` (`POL-REJECT-INVALID`). Per-file sheets were written for the review and reject files only. Report: [UC06.md](https://github.com/shuji-bonji/pdf-agent-stack/blob/main/eval/hosts/grok-build/reports/UC06.md) (Japanese)
