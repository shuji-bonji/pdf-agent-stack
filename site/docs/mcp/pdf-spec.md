...
::: info The specification PDFs are not bundled
Place the freely available originals in `PDF_SPEC_DIR` yourself (→ [Getting started, Step 2](/guide/getting-started)). **Every core document is available at no cost through legitimate channels.** Without the corpus this server cannot answer a single question.
:::
...
On its own, the central use case is [spec research](/use-cases/spec-research).

## What it cannot do

- **It cannot reach outside the corpus.** ISO 19005 (PDF/A) and ETSI PAdES are not included (`list_specs` → `coverage.gaps`). **Zero search hits ≠ no such requirement exists**
- **It can say nothing about your file.** What comes back is the text of the standard; whether a given PDF satisfies it is pdf-verify's answer
...
The output falls into three kinds.
...
- **Definitions** (`get_definitions`) — term definitions from Section 3 (Terms and definitions). The further a term drifts from everyday usage, the more it pays to settle it here before arguing
